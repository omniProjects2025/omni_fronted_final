...
export class BlogsComponent {
  ...
  getBlogsDetails(): void {
    this.http
      .get<any>('assets/json_data_files/main_blogs.json')
      .subscribe((json_data) => {
        this.allBlogs = json_data.blogs;
        this.allBlogs.sort((a: any, b: any) => {
  ...
  updateDisplayedBlogs(): void {
    let filtered = this.selectedCategory
      ? this.allBlogs.filter(
          (blog) => blog.category_name === this.selectedCategory,
        )
      : this.allBlogs;
  ...
  private setSEOTags(): void {
    this.titleService.setTitle(
      'OMNI Hospitals Blog | Health News & Expert Medical Advice',
    );
    this.metaService.updateTag({
      name: 'description',
      content:
        'Read the OMNI Hospitals Blog for expert articles on health, wellness, and medical conditions. Get reliable advice from our top doctors and specialists.',
    });
